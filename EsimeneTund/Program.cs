using EsimeneTund.Ulessane_listid;
using System.Text;
...
internal class Program
{
    public static void Main(string[] args) {
        ...
        List<Toode> toidud = [
            new Toode("Õun", 52), new Toode("Banaan", 89), new Toode("Kana", 239), new Toode("Leib", 265)
        ];

        var inim = Funktsionid.DataAsk();
        Funktsionid.Matem(inim, toidud);

        Dictionary<string, string> maakondid = new Dictionary<string, string>();

        maakondid.Add("Harjumaa", "Tallinn");
        maakondid.Add("Tartumaa", "Tartu");
        maakondid.Add("Pärnumaa", "Pärnu");
        maakondid.Add("Ida-Virumaa", "Jõhvi");
        maakondid.Add("Läänemaa", "Haapsalu");

        Console.WriteLine("Sisesta maakond: ");
        string sisestus = Console.ReadLine();

        var found = "";
        foreach (var maakond in maakondid)
        {
            if (sisestus.ToLower() == maakond.Value.ToLower())
            {
                found = maakond.Key;
                break;
            }
        ...
        {
            Console.WriteLine($"Maakond on {found}");
        }
        ...
        //Faili_tootamine.Ridade_lugemine("Kuud.txt");

        //Faili_tootamine.Faili_lugemine("Kuud.txt");

        //Faili_tootamine.KirjutaFail();
        ...
        //Class3.KuniMärksõnani("elephant", "Osta elephanti: ");
